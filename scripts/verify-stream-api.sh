...
if (($# != 0)); then
  printf 'usage: scripts/verify-stream-api.sh\n' >&2
  exit 1
fi
...
# Rename lane: the public API carries the stream
...
# Standard-body lane: bodies are futures-core Streams in
...
# Two-phase contract lane: the external-crate proof drives
