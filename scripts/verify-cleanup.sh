...
if (($# != 0)); then
  printf 'usage: scripts/verify-cleanup.sh\n' >&2
  exit 1
fi
...
# Convergence lane: the tombstone converges through ordinary sync, excludes
# the subject from sessions and re-merges, and the binding stays as
# permanent evidence. The purge lane below clears one local record
# explicitly and restores the local session boundary.
cargo test --locked --all-features --test cleanup -- --list > "$TMP/cleanup-it.list"
require_lane "convergent cleanup" 'cleanup_converges_and_excludes_the_subject' "$TMP/cleanup-it.list"
require_lane "purge" 'purge_revocation_clears_the_local_boundary' "$TMP/cleanup-it.list"
cargo test --locked --all-features --test cleanup
# These cleanup lanes also carry the checkpoint-GC scenarios: the max-wins
# watermark convergence runs through the facade in tests/cleanup.rs and the
# sweep/filter boundary in the unit module of identity::cleanup above.
