...
if (($# != 0)); then
  printf 'usage: scripts/verify-revocation.sh\n' >&2
  exit 1
fi
...
# Record lane: the signed tombstone commits
...
# Convergence lane: the expulsion reaches every member —
...
require_lane "cluster-wide expulsion" 'delayed_content_converges_after_revoke' "$TMP/rev-it.list"
require_lane "session denial" 'revoke_closes_sessions_denies_reconnect_and_preserves_metadata' "$TMP/rev-it.list"
cargo test --locked --all-features --test revocation

# Permanence lane: the revocation record family rides the
# sync forwarder alongside leave/cleanup with the shared bounds.
cargo test --locked --all-features --lib membership::sync

printf 'VERIFY-REVOCATION PASS\n'
