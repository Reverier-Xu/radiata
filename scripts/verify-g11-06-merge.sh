...
if (($# != 0)); then
  printf 'usage: scripts/verify-g11-06-merge.sh\n' >&2
  exit 1
fi
...
# Record lane (SC-G11-P0-12): the merge triple commits atomically, replays
...
# Credential lane (SC-G11-P0-11): wrong credential, single-use, rate window,
...
# Union lane (SC-G11-P0-13): a sixteen-node merge chain converges every
...
# Wire lane (SC-G11-P0-14): no cluster field survives in handshake, hint,
