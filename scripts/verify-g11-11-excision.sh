...
if (($# != 0)); then
  printf 'usage: scripts/verify-g11-11-excision.sh\n' >&2
  exit 1
fi
...
# Excision lane (SC-G11-P0-31): no cluster identifier survives in code,
...
# Golden lane (SC-G11-P0-32): the regenerated vectors pin the rebaselined
...
# Mixed-version lane (SC-G11-P0-33): the rebaselined formats pass the
# mixed-binary compatibility suite.
cargo test --locked --all-features --test mixed_versions

printf 'VERIFY-G11-11 PASS\n'
