...
| 9 | **离开的身份永远留在成员页**：`PageMembers` 不过滤 left 身份，成员计数含死记录，且无公开 API 能区分哪个成员已离开——客户无法从公共面计算"存活成员数"。生命周期实测：leave 后 members 仍为 10（9 活 + 1 左）。 | 中 | 建议成员视图暴露 left/cleaned 状态或提供存活过滤选项 |
| 10 | **同步游标跳过连接空窗期的写入（P1）**：资源/描述符反熵的 per-peer keyset 游标在会话死亡后仍保留，重连后的新会话从旧游标继续——网络分区/重挂期间写入的数据对游标已越过的对端永不重传。生命周期实测：分区写入 split-b 在治愈后 182s+ 仍停留在写入者本地，冲突寄存器两侧各自持不同胜者无法归一（6:1:2 分布）。 | 高 | 修复方向：以每键摘要/revision 水位替代名字游标，或会话重建时重置该对端的游标 |
| 9 | **离开的身份永远留在成员页**：`PageMembers` 不过滤 left 身份，成员计数含死记录，且无公开 API 能区分哪个成员已离开——客户无法从公共面计算“存活成员数”。生命周期实测：leave 后 members 仍为 10（9 活 + 1 左）。 | 中 | 建议成员视图暴露 left/cleaned 状态或提供存活过滤选项 |
| 10 | **同步游标跳过连接空窗期的写入（P1）**：资源/描述符反熵的 per-peer keyset 游标在会话死亡后仍保留，重连后的新会话从旧游标继续——网络分区/重挂期间写入的数据对游标已越过的对端永不重传。生命周期实测：分区写入 split-b 在治愈后 182s+ 仍停留在写入者本地，冲突寄存器两侧各自持不同胜者无法归一（6:1:2 分布）。 | 高 | 修复方向：以每键摘要/revision 水位替代名字游标，或会话重建时重置该对端的游标 |
...
## 本轮已落地（cluster-lifecycle → fix-sync-cursors）

1. **join hint 竞态（库缺陷）**：监听器现在每连接评估凭据世代 hint（`TransportListener::accept` 改收 hint 回调，crate 内部 trait 变更），轮换期间的 join 不再被过期 hint 毒化。
2. **同步游标 per-peer 化**：资源与成员反熵的页游标/指纹/重发节拍全部按对端独立跟踪；对端离开活跃集即 prune（重入时全量补发），周期性 from-scratch pass 兑现 finding #10 的有界丢失窗口承诺。
3. **测试基建**：16 节点拓扑断言改限期轮询；admission 的 StorageLocked 竞态限期重试；新增资源同步 gap-write 回归测试（finding #10 的单位级钉子）。
