...
1. **单流突发上限 = 会话队列窗口（16 MiB）**。数据面对源头 chunk 无阻塞背压：队列满时 `send` 返回 `Overloaded`，泵将该流以 `StreamInterrupted` 终止（文档化的"显式中断、永不重放"语义）。因此**单条流一次性突发超过 16 MiB 必被截断**——这不是缺陷而是流控设计，但调用方（以及吞吐基准）必须按"多条流、每条 ≤ 窗口"的方式发送。上表 radiata 吞吐即按此口径（可持续吞吐）。
